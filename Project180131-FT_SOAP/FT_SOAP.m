% Branch from Hermit_coeff_NOR_RV_correct_0720.m
% Stop developping; refer to the more advanced version in Project180201-FT_SOAP

% Use simulated spectra with planets AND stellar jitter: 
% /Volumes/DataSSD/SOAP_2/outputs/HERMIT_2spot_0720/CCF_dat/
...
dir1            = '/Volumes/DataSSD/SOAP_2/outputs/HERMIT_2spot_0720/';
dir2            = '/Volumes/DataSSD/SOAP_2/outputs/HERMIT_2spot_0720/CCF_dat/';
...
filename    = [dir2, 'CCF', num2str(0), '.dat'];
...
for n = 1:N_FILE
    ...
    i           = n - 1;
    v_planet    = 100 * sin(i/25*0.618*2*pi + 1) * 0.001;         % km/s
...
array = 1:N_FILE;
[pxx_fft,f_fft] = plomb(FFT_power(1, :)', array, 0.5,  'normalized');
...
for j = 1:idx_max
    ...
    [pxx_fft,f_fft] = plomb(FFT_power(j, :)', array, 0.5,  'normalized');
    Z_fft(j,:) = pxx_fft;
