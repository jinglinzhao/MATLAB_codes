...
% Parameters %
%%%%%%%%%%%%%%
DIR         = '/Volumes/DataSSD/OneDrive - UNSW/Hermite_Decomposition/ESO_HARPS/HD85390';
...
% Phase angle -> RV %
%%%%%%%%%%%%%%%%%%%%%
n   = (FFT_frequency > -0.2) & (FFT_frequency < 0.2);
...
%     errorbar(MJD, RV_FT, RV_noise , 'b.', 'MarkerSize', 20)
    errorbar(MJD, (RV_FTH-RV_FTL)*1000, RV_noise , 'b.', 'MarkerSize', 20)
    grid on
    hold off
    xlabel('Time [d]')
    ylabel('RV [m/s]')
...
width   = 300;
idx     = MJD < 57300;
jitter_proto = (RV_FTH(idx) - RV_FTL(idx))*1000;
MJD1    = MJD(idx);
