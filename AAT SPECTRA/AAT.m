...
%     loc = './AAT_Spec/';
    loc = './AAT_Work/';
    file_list   = dir([loc, dir_list{n_dir}]);
    ...
    max_SN      = 0;
    SN_array     = zeros(1, N_file);
    for n_file = 1 : N_file
        file    = [file_list(n_file).folder, '/', file_list(n_file).name];
...
%             telluric_max    = max(telluric_max, telluric); 
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%
        if 1
            telluric        = telluric * (bc_RV / c + 1); %jzhao
            %%%%%%%%%%%%%%%%%%%%%%%%%
...
%                 f = fit( lag', int_cor, ft, 'StartPoint', [SPACING, 0, max(int_cor)] );
        ...
        if (min(int_nor(:,n_file))<0.5) && (SN_array(n_file)>30)
...
%     elseif (w <= 1) 
    area([telluric_left telluric_right], [0 0], 'LineStyle', 'none', 'FaceColor','r', 'FaceAlpha', 0.1);
    area([telluric_left telluric_right], [1.05 1.05], 'LineStyle', 'none', 'FaceColor','r', 'FaceAlpha', 0.1);
...
%     end 
    ...
    elseif cmd_subplot
        ...
        hb = figure;
%         set(hb,'PaperSize',[10 8]);
    end
%     hold on
%     yyaxis left

%     for n_file = 1 : N_file
%         if ~(median(int_nor(:,n_file))==1) && (min(int_nor(:,n_file))<0.5)
%         plot(wav(:,n_file), int_nor(:,n_file))
%         errorbar(bary_JD(n_file), EW(n_file), delta_EW(n_file), 's', 'markers', 8, 'MarkerFaceColor', 'b')
%     end
    idxxx = (min(int_nor)<0.5) & (SN_array>30);
    errorbar(bary_JD(idxxx), EW(idxxx), delta_EW(idxxx), 's', 'markers', 5, 'MarkerFaceColor', 'b')
    ylabel('EW (\AA)', 'Interpreter', 'latex')
    % for n_file = 1 : N_file
    %   plot(bary_JD(n_file), EW(n_file), '^', 'markers', 8, 'color', cc(n_file,:))
    % end
%     y_min = min(EW - 1.1*delta_EW);
%     y_max = max(EW + 1.1*delta_EW);
%     y_min = y_min - y_max;
%     ylim([y_min y_max])
...
%     hold off
    ...
    if cmd_eps
        out_eps = [output, '_b.pdf'];
        print(out_eps, '-dpdf', '-bestfit')
        close(hb);
    end                  
...
%     end
    ...
    if 0
        ...
        if 0
            ...
            hold off
            % pic_name    = ['FLX_DIFF', dir_list{n_dir}(1:end-1), '_w', num2str(w), '.pdf'];
            % print(pic_name, '-dpdf')  
            % close(h_diff);
            xlabel('RV (m/s)', 'Interpreter','latex')
            ylabel('LPD', 'Interpreter','latex')  
    ...
    elseif cmd_subplot
        eps_name    = ['EW_RV', dir_list{n_dir}(1:end-1), '_w', num2str(w), '.pdf'];
        print(eps_name, '-dpdf')
    end
    close(h_show);
...
if cmd_scatter
    ...
    pic_name = ['scatter_', num2str(w), '.pdf'];
    print(pic_name, '-dpdf')
    % close(h_scatter)
end
...
if cmd_scatter2
    ...
    % saveas(h_scatter, pic_name);
    pic_name = ['scatter2_', num2str(w), '.pdf'];
    print(pic_name, '-dpdf')
    % close(h_scatter2)
end
